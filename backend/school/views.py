...
@csrf_exempt
def manage_cohort(request, id_cohort=None):
    ...
    if request.method == 'GET':
        ...
        try:
            if id_cohort:
                ...
                cohort_dict = model_to_dict(cohort)
                return JsonResponse({'success': True, 'data': cohort_dict}, status=200)
            else:
                cohorts = Cohort.objects.all()
                cohorts_list = [model_to_dict(cohort) for cohort in cohorts]
                return JsonResponse({'success': True, 'data': cohorts_list}, status=200)
        except Cohort.DoesNotExist:
            return JsonResponse({'success': False, 'message': 'No cohort found'}, status=404)
        ...
        try:
            # Define unique fields for the Cohort model
            unique_fields = {'cohort_name': data.get(
                'cohort_name'), 'cohort_year': data.get('cohort_year')}

            # Check if a cohort with the same unique fields already exists
            if Cohort.objects.filter(**unique_fields).exists():
                return JsonResponse({'success': False, 'message': 'Cohort with the same name and start date already exists'}, status=400)

            # Create the new Cohort
            cohort = Cohort.objects.create(**data)
            if cohort:
                return JsonResponse({'success': True, 'message': 'Cohort added successfully'}, status=201)
        except Exception as e:
        ...
        try:
            # cohort = Cohort.objects.filter(pk=id_cohort).update(**data)
            # if cohort:
            #     return JsonResponse({'success': True, 'message': 'Cohort updated successfully'}, status=200)

            # Extract the cohort ID and remove it from the data dictionary
            id_cohort = data.pop('id_cohort', None)

            if not id_cohort:
                return JsonResponse({'success': False, 'message': 'Cohort ID is required'}, status=400)

            # Update the Cohort object
            updated_count = Cohort.objects.filter(pk=id_cohort).update(**data)

            if updated_count:
                return JsonResponse({'success': True, 'message': 'Cohort updated successfully'}, status=200)

            else:
                return JsonResponse({'success': False, 'message': 'Cohort not found'}, status=404)
        except Exception as e:
            return JsonResponse({'success': False, 'message': str(e)}, status=400)
        ...
        try:
            ...
            if cohort[0]:
                return JsonResponse({'success': True, 'message': 'Cohort deleted successfully'}, status=200)
            else:
                return JsonResponse({'success': False, 'message': 'Cohort not found'}, status=404)
        except Exception as e:
            return JsonResponse({'success': False, 'message': str(e)}, status=400)
    ...
    if request.method == 'GET':
        ...
        try:
            if id_classe:
                ...
                return JsonResponse({'success': True, 'data': classe_dict}, status=200)
            else:
                classes = Classe.get_all()
                classes_list = [model_to_dict(classe) for classe in classes]
                return JsonResponse({'success': True, 'data': classes_list}, status=200)
        ...
        try:
            ...
            if not cohort:
                return JsonResponse({'success': False, 'message': 'Invalid cohort'}, status=400)
            ...
            if Classe.objects.filter(**unique_fields).exists():
                return JsonResponse({'success': False, 'message': 'Classe with the same name already exists for this cohort'}, status=400)
            ...
            if classe_created:
                return JsonResponse({'success': True, 'message': 'Classe added successfully'}, status=201)
            else:
                return JsonResponse({'success': False, 'message': 'Failed to add classe'}, status=400)
        except Exception as e:
            return JsonResponse({'success': False, 'message': str(e)}, status=400)
        ...
        try:
            ...
            if classe:
                return JsonResponse({'success': True, 'message': 'Classe updated successfully'}, status=200)
            else:
                return JsonResponse({'success': False, 'message': 'Classe not found'}, status=404)
        except Exception as e:
            return JsonResponse({'success': False, 'message': str(e)}, status=400)
        ...
        try:
            classe = Classe.objects.filter(
                pk=id_classe).delete()
            if classe[0]:
                return JsonResponse({'success': True, 'message': 'Classe deleted successfully'}, status=200)
            else:
                return JsonResponse({'success': False, 'message': 'Classe not found'}, status=404)
        except Exception as e:
            return JsonResponse({'success': False, 'message': str(e)}, status=400)
    ...
    if request.method == 'GET':
        ...
        try:
            if id_material:
                ...
                return JsonResponse({'success': True, 'data': material_dict}, status=200)
            else:
                materials = Material.objects.all()
                materials_list = [model_to_dict(material)
                                  for material in materials]
                return JsonResponse({'success': True, 'data': materials_list}, status=200)
        except Material.DoesNotExist:
            return JsonResponse({'success': False, 'message': 'No material found'}, status=404)
        except Exception as e:
            return JsonResponse({'success': False, 'message': f'Error: {e}'}, status=500)
        ...
        try:
            ...
            if Material.objects.filter(category=category, item=item).exists():
                return JsonResponse({'success': False, 'message': 'Item already exists in this category'}, status=400)

            material = Material.create(**data)
            if material:
                return JsonResponse({'success': True, 'message': 'Material added successfully'}, status=201)
        except json.JSONDecodeError:
            return JsonResponse({'success': False, 'message': 'Invalid JSON data'}, status=400)
        except Exception as e:
            return JsonResponse({'success': False, 'message': str(e)}, status=400)
        ...
        try:
            ...
            if material:
                return JsonResponse({'success': True, 'message': 'Material updated successfully'}, status=200)
            else:
        ...
        try:
            ...
            if material:
                return JsonResponse({'success': True, 'message': 'Material deleted successfully'}, status=200)
            else:
                return JsonResponse({'success': False, 'message': 'Material not found'}, status=404)
        except Exception as e:
            return JsonResponse({'success': False, 'message': str(e)}, status=400)
    ...
    if request.method == 'GET':
        ...
        try:
            if id_salle:
                ...
                return JsonResponse({'success': True, 'data': salle_dict}, status=200)
                ...
                return JsonResponse({'success': True, 'data': salles_list}, status=200)
        except (SalleFormation.DoesNotExist, MaterialSalleFormation.DoesNotExist):
            return JsonResponse({'success': False, 'message': 'No salle or material salle formation found'}, status=404)
        except Exception as e:
            return JsonResponse({'success': False, 'message': f'Error: {e}'}, status=500)
        ...
        try:
            ...
            if SalleFormation.objects.filter(salle_name=salle_name, etage=etage).exists():
                return JsonResponse({'success': False, 'message': 'Salle with the same name and floor already exists'}, status=400)
            ...
            return JsonResponse({'success': True, 'message': 'Salle formation added successfully'}, status=201)
        ...
        try:
            ...
            if salle_id:
                ...
                return JsonResponse({'success': True, 'message': 'Salle formation updated successfully'}, status=200)
            else:
                return JsonResponse({'success': False, 'message': 'Salle ID not provided'}, status=400)

        except SalleFormation.DoesNotExist:
            return JsonResponse({'success': False, 'message': 'Salle formation not found'}, status=404)
        except Exception as e:
        ...
        try:
            ...
            if salle:
                return JsonResponse({'success': True, 'message': 'Salle formation deleted successfully'}, status=200)
            else:
                return JsonResponse({'success': False, 'message': 'Salle formation not found'}, status=404)
        except Exception as e:
            return JsonResponse({'success': False, 'message': str(e)}, status=400)
    ...
    if request.method == 'GET':
        ...
        try:
            if id_seance:
                ...
                return JsonResponse({'success': True, 'data': seance_dict}, status=200)
            else:
                seances = Seance.objects.all()
                seances_list = [model_to_dict(seance) for seance in seances]
                return JsonResponse({'success': True, 'data': seances_list}, status=200)
        ...
        try:
            ...
            try:
                ...
                return JsonResponse({'success': False, 'message': 'No salle_formation found'}, status=404)
            ...
            if Seance.objects.filter(
                ...
                return JsonResponse({'success': False, 'message': 'Seance with the same details already exists'}, status=400)
            ...
            if seance:
                return JsonResponse({'success': True, 'message': 'Seance added successfully'}, status=201)
            else:
                return JsonResponse({'success': False, 'message': 'Failed to add seance'}, status=400)

        except KeyError as e:
            missing_field = e.args[0]
            return JsonResponse({'success': False, 'message': f'Missing field: {missing_field}'}, status=400)
        ...
        try:
            ...
            return JsonResponse({'success': True, 'message': 'Seance updated successfully'}, status=200)

        except Seance.DoesNotExist:
            return JsonResponse({'success': False, 'message': 'Seance not found'}, status=404)

        except (Classe.DoesNotExist, Teacher.DoesNotExist, Matiere.DoesNotExist, SalleFormation.DoesNotExist) as e:
            return JsonResponse({'success': False, 'message': 'Related object not found'}, status=404)
        ...
        try:
            ...
            if seance[0]:
                return JsonResponse({'success': True, 'message': 'Seance deleted successfully'}, status=200)
            else:
                return JsonResponse({'success': False, 'message': 'Seance not found'}, status=404)
        except Exception as e:
            return JsonResponse({'success': False, 'message': str(e)}, status=400)
...
#                 return JsonResponse({'success': True, 'data': material_salle_dict}, status=200)
#             else:
#                 material_salles = MaterialSalleFormation.objects.all()
#                 material_salles_list = [model_to_dict(
#                     msf) for msf in material_salles]
#                 return JsonResponse({'success': True, 'data': material_salles_list}, status=200)
#         except MaterialSalleFormation.DoesNotExist:
#             return JsonResponse({'success': False, 'message': 'No material salle formation found'}, status=404)
#         except Exception as e:
#             return JsonResponse({'success': False, 'message': f'Error: {e}'}, status=500)
...
#                 return JsonResponse({'success': False, 'message': 'Invalid salle formation or material'}, status=400)
...
#                 return JsonResponse({'success': True, 'message': 'Material salle formation added successfully'}, status=201)
#             else:
#                 return JsonResponse({'success': False, 'message': 'Failed to add material salle formation'}, status=400)
#         except json.JSONDecodeError:
#             return JsonResponse({'success': False, 'message': 'Invalid JSON data'}, status=400)
#         except Exception as e:
#             return JsonResponse({'success': False, 'message': str(e)}, status=400)
...
#             if material_salle:
#                 return JsonResponse({'success': True, 'message': 'Material salle formation updated successfully'}, status=200)
#             else:
#                 return JsonResponse({'success': False, 'message': 'Material salle formation not found'}, status=404)
#         except Exception as e:
#             return JsonResponse({'success': False, 'message': str(e)}, status=400)
...
#             if material_salle:
#                 return JsonResponse({'success': True, 'message': 'Material salle formation deleted successfully'}, status=200)
#             else:
#                 return JsonResponse({'success': False, 'message': 'Material salle formation not found'}, status=404)
#         except Exception as e:
#             return JsonResponse({'success': False, 'message': str(e)}, status=400)
